...
var wacomstu540 = function () {
  ...
  this.setPenColorAndWidth = async function (color, width) {
    ...
    let c = color
      .replace("#", "")
      .split(/(?<=^(?:.{2})+)(?!$)/)
      .map((e) => parseInt("0x" + e, 16)); //Converts "#RRGGBB" to Array(r,g,b)
  ...
  this.setImage = async function (imageData) {
    ...
    await this.sendData(
      this.command.writeImageStart,
      new Uint8Array([this.config.imageFormat24BGR]),
    );
    this.image.forEach(
      async function (e) {
        await this.sendData(
          this.command.writeImageData,
          new Uint8Array([e.length, 0].concat(e)),
        );
      }.bind(this),
    );
  ...
  this.dataViewString = function (dv, offset, length) {
    var end = typeof length == "number" ? offset + length : dv.byteLength,
      text = "",
      val = -1;
  ...
  navigator.hid.addEventListener(
    ...
    function (e) {
      if (
        this.config.onHidChangeCb != null &&
